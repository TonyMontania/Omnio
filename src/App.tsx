                    ...
                    <div className="field-group">
                      ...
                      <div className="about-box">
                        ...
                        <ul className="about-changelog">
                          <li><b>Inline-editable tracklist</b> — number, title, artist and duration are all editable in place across the main tracklist and every edition tracklist. Same for the tracklists inside the edition editor.</li>
                          <li><b>First-run wizard</b> — new installs get a welcome screen with three CTAs (import an existing library, set up metadata API keys, or pick a category and add the first item). Dismissed permanently on first choice.</li>
                          <li><b>Per-category cover placeholders</b> — items without a cover now show a category-specific SVG glyph (controller for Games, cassette for Music, clapperboard for Movies, TV for Series, play triangle for Anime / Donghua, stacked panels for the Manga family, open book for Books) instead of the "No cover" text.</li>
                          <li><b>Three new importers</b> — Backloggd (Games CSV), Serializd (Series JSON or CSV), Spotify library (YourLibrary.json → albums). All follow the same dedupe-against-existing / preselect-new / per-row-checkbox pattern the other importers use.</li>
                          <li><b>CSV export</b> — per-category CSV writer in addition to the HTML site / MAL XML / iCal exports. One file per category with common core fields plus category-specific columns. Music always ships an extra <code>omnio-artists.csv</code>.</li>
                          <li><b>Metadata search cache</b> — every search fetcher (MB, Jikan, Kitsu, MangaDex, VGMdb, ComicVine, IGDB, TMDb, AniList, OpenLibrary) caches results by query for 24h under <code>data/cache/searches.json</code>. Repeat searches feel instant and respect MB's 1-req/s rate limit for free.</li>
                          <li><b>HTTP proxy setting</b> — new field under Settings → Integrations & network routes every outbound request through an HTTP(S) proxy. Useful for NAS containers behind corporate firewalls or Pi-hole. Applied on blur; persists across launches.</li>
                          <li><b>Docker linux/arm64</b> — the published GHCR image is now a multi-arch manifest. Raspberry Pi 5, ARM NASes, Apple Silicon Docker Desktop and Graviton hosts pull the right slice automatically.</li>
                          <li><b>Reorganized Settings</b> — the sidebar groups tabs by area (Look &amp; feel, Libraries, Data) and the old catch-all Data tab is split into Backup / Integrations & network / Maintenance & about, each focused on one thing.</li>
                          <li><b>Lint gate + smoke test in CI</b> — every push runs <code>eslint --max-warnings 0</code> and a Playwright + Electron smoke test that boots the app and verifies the renderer mounts. Regressions in main.ts / preload / vite bundle surface immediately on PR.</li>
                          <li><b>Music + Games editors extracted</b> — the two biggest editor JSX blocks moved out of <code>App.tsx</code> into their own components. -212 net lines in App.tsx; state stays hoisted so no behavior changes.</li>
                          <li><b>Global click-to-zoom on covers, banners and logos</b> — full-screen lightbox on every image across every detail view.</li>
                          <li><b>Music polish</b> — EP ↔ album link picker, edition release dates + cover fallback, single covers rendered at album size, taller artist banner (470px). MusicBrainz and VGMdb fetchers pull producers / genres / distributors / full release dates.</li>
                        </ul>
